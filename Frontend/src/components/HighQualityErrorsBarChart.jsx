 ...
 */
const HighQualityErrorsBarChart = ({ limit = 50 }) => {
    ...
    const fetchDetectedErrors = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8000/api/results/high_quality_errors?limit=${limit}`
        );
        ...
        console.error('Error fetching high quality errors:', err);
  ...
  return (
    <ResponsiveContainer width="100%" height={400}>
      ...
      >
        ...
        />
        <Bar dataKey="high_quality_errors" fill="#3b82f6" name="High Quality Errors" />
      </BarChart>
    </ResponsiveContainer>
  );
};

export default HighQualityErrorsBarChart;
