...
def insert_configurations(system_prompt, model, project_id):
    ...
    Args:
        ...
        project_id (int): The project ID this configuration belongs to
...
def insert_fixes(number_of_fixes, duration, tokens, project_id, config_id):
    ...
    Args:
        number_of_fixes (int): Number of fixes
...
def get_config_results(project_id):
    """
    Fetch all configurations and results for a given project.
    
    Args:
        project_id (int): The project ID to fetch data for
        
    Returns:
        list: Array of configuration and result data
    ...
    try:
        ...
        raise psycopg2.Error(f"Failed to fetch configuration results: {e}")
    finally:
        if cur:
            cur.close()
        if conn:
...
def insert_new_system_prompts(projectid: int, prompt):
    """
    Insert a new system prompt configuration.
